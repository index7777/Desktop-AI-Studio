...
MODEL_ID = os.getenv("AI_STUDIO_MODEL", "Qwen/Qwen-Image-2.1")
...
class QwenEngine:
    ...
    def load(self, progress: Callable[[str], None] | None = None) -> None:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("Qwen Image 2.1 MVP requires a CUDA-capable NVIDIA GPU.")
        if progress: progress("loading-model")
        self.pipe = QwenImage21Pipeline.from_pretrained(MODEL_ID, dtype=torch.bfloat16).to("cuda")
    ...
    def generate(self, req: GenerateRequest, progress: Callable[[str], None] | None = None) -> dict:
        ...
        kwargs = dict(prompt=req.prompt, width=req.width, height=req.height,
        ...
        return {"path": str(Path(req.output_path).resolve()), "seed": seed,

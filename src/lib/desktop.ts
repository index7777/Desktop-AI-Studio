export type HardwareStatus={gpuName:string;vramTotalMb:number|null;vramFreeMb:number|null;nvidiaAvailable:boolean;engineState:string}; export type GenerateRequest={prompt:string;width:number;height:number;steps:number;seed:number}; export type GenerateResult={imageDataUrl:string;path:string;seed:number;elapsedSeconds:number;width:number;height:number};
